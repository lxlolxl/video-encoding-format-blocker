(() => {
  ...
  const BLOCKED_CODECS = Object.freeze(['av1']);
  ...
  function hasBlockedCodec(value) {
    if (typeof value !== 'string') return false;
    return BLOCKED_CODECS.some(codec => tests[codec].some(re => re.test(value)));
  }

  function scanConfig(value, depth = 0, seen = new Set()) {
    if (value == null || depth > 5) return false;
    if (typeof value === 'string') return hasBlockedCodec(value);
    if (typeof value !== 'object' || seen.has(value)) return false;
    seen.add(value);
    for (const key of ['contentType', 'mimeType', 'type', 'codecs', 'codec']) if (hasBlockedCodec(value[key])) return true;
    return scanConfig(value.video, depth + 1, seen) || scanConfig(value.audio, depth + 1, seen) || scanConfig(value.config, depth + 1, seen);
  }

  function patch(owner, name, factory) {
    try {
      if (!owner || typeof owner[name] !== 'function') return;
      const original = owner[name];
      Object.defineProperty(owner, name, { value: factory(original), configurable: true, writable: true });
    } catch (_) {}
  }
  ...
  if (navigator.mediaCapabilities) {
    patch(navigator.mediaCapabilities, 'decodingInfo', original => function(config) { return scanConfig(config) ? Promise.resolve({ supported: false, smooth: false, powerEfficient: false, keySystemAccess: null }) : original.apply(this, arguments); });
    patch(navigator.mediaCapabilities, 'encodingInfo', original => function(config) { return scanConfig(config) ? Promise.resolve({ supported: false, smooth: false, powerEfficient: false }) : original.apply(this, arguments); });
  }
